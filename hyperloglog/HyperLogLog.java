...
public class HyperLogLog {
    public static void main(String[] args) {        
        ...
        while (scanner.hasNextInt()) {
            ...
            inputData.add(Integer.parseUnsignedInt(input));
        }

        scanner.close();

        int m = Integer.parseInt(args[0]);
        double result = hyperLogLog(inputData, m);
        double difference = Math.abs(inputData.size() - result);
        System.out.println(inputData.size() + "," + result + "," + difference);
    }
    ...
    public static int hB(int x, int m) {
        int shift_amount = 31 - (int)(Math.log(m) / Math.log(2));
        return ((x * 0xbc164501) & 0x7fffffff) >> shift_amount;
    }
